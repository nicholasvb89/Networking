...
def client_sender(buffer):
    ...
    client = socket.socket(socket.AF_INET, socker.SOCK_STREM)
...
def server_loop():
    ...
    while True:
        clinet_scoket, addr = server.accept()

        #spin off a thread to handle our new client

        clinet_thread = threading.Thread(target=client_handler, args=(client_socket,))
        client_thread.start()
...
def clinet_handler(client_socket):
    global upload
    global execute
    global command
    ...
    if len(upload_destination):
        ...
        #read in all the bytes and write to our detination
        file_buffer = ""

        #keep reading data until none is avaialbe
        while True:
        ...
        try:
            ...
            client_socket.send(f"Sucessfully Saved file to {upload_destination}\r\n")
        except:
    ...
    #now we go into another loop if a command shell was requests
    if command:
        while True:
